...
- 配置标的、时间窗口、初始资金和决策间隔；
- 提交后先验证共同交易日；`NEXT_OPEN` 至少需要两个，周末和休市日不计入；
- 显式配置手续费、最低费用和滑点；
- 真实模式可选择参与图运行的分析师与单标的建仓上限；
- 默认每个共同交易日决策一次；也可显式配置低频策略间隔；
- 提交后明确显示预计完整 Agent 图调用次数，不再用隐藏上限改变决策频率；
- 实时查看 `RunEvent` 进度流水线；
- 运行成功或失败都会写入 SQLite。
...
Manager，避免 Agent 在空仓时误写“维持现有仓位”。
...
- 真实模式默认日频决策，每个交易日都会运行完整多 Agent 图；这比演示策略慢得多，
  长窗口可能产生大量模型请求。页面会明确显示预计调用数，但不会暗中把日频改成
  30 bars，也不会再用 12 次硬上限拒绝运行；
- 首次加载本地 embedding 可能需要下载模型；一次完整图调用可能以分钟计，而不是
  演示策略的秒级执行。低频间隔仍可作为用户主动选择的策略参数；
