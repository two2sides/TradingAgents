...
- [x] 提供 Yahoo Chart 共享磁盘缓存适配器；保留 yfinance 兼容入口。
...
默认仓位政策使用 35% 单标的仓位上限，并同时应用 `1/N` 分散上限。评级映射为
绝对目标暴露，使空仓账户也能执行中性和防御评级：

- `Buy`：目标为上限的 100%；
- `Overweight`：目标为上限的 75%；
- `Hold`：目标为上限的 50%；
- `Underweight`：目标为上限的 25%；
- `Sell`：目标仓位为 0。

这不是 C 的内部实现假设，而是 C 的五级评级与 A 的连续目标仓位契约之间必须
存在的明确执行政策。A 会把实际组合状态和映射档位注入 Trader 与 Portfolio
Manager；参数可以替换，默认值不会藏在 Prompt 或 WebUI 中。
