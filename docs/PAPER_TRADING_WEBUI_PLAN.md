...
- [ ] 用不可变 `LedgerEntry` 保存入金、成交、费用和估值事件。
- [ ] 从账本重建现金、数量、平均成本和净值。
- [ ] 将目标仓位转换为整数股买卖。
- [ ] 支持手续费、最低费用、滑点、现金不足和明确拒单。
- [ ] 测试账本可重放以及所有账户硬约束。
...
- [ ] 使用 SQLite 保存请求、结果、账本和进度事件。
- [ ] 支持运行列表、详情、删除和导出。
- [ ] 支持复用已有 Agent 决策的“仅执行层 What-if”。
- [ ] 明确区分完整 Agent 重跑与近似的执行层反事实实验。
...
- [ ] 建立深石墨、青蓝和琥珀色视觉系统。
- [ ] 回测表单展示费用、滑点、时间窗口和 Provider 信息。
- [ ] 实时展示阶段、进度、当前日期和降级警告。
- [ ] 在同一时间轴展示 K 线、交易、仓位、净值和回撤。
- [ ] 点击决策点查看 Agent 结论、记忆、成交和后续表现。
- [ ] 支持两次运行并排比较以及执行层 What-if。
- [ ] 提供无需 API Key 的确定性演示模式。

验收：首次使用者不阅读代码即可完成“发起回测 → 定位一笔交易 → 解释结果 → 比较实验”流程。

### M5：集成与演示加固

- [ ] 用 B/C 的 Mock 完成全链路契约测试。
- [ ] 接入真实 B/C 实现时不修改 A 的 Broker 和 WebUI。
- [ ] 准备一个短周期快速演示数据集和一个完整结果样例。
- [ ] 补充启动命令、故障提示和演示说明。
