...
通过 config 的 `memory_embedding` 键切换。默认使用本地模型。
...
- `_run_graph`：provider 激活时，调用 `_retrieve_agent_memories()` 为 7 个角色（PM、市场、基本面、情绪、新闻、多头、空头）并行检索记忆，合并到初始状态。
- `_record_decision_via_provider`：图运行结束后，将决策写入 RAG store。

修改了 `tradingagents/graph/propagation.py`：

- `create_initial_state` 增加 `**extra_state` 参数，允许注入 `memory_{role}` 字段而不破坏签名兼容性。
...
| 文件 | 行数 | 说明 |
|---|---|---|
| `tradingagents/extensions/memory/agent_profiles.py` | ~200 | 12 角色检索配置 + 市场特征提取 |
| `tradingagents/extensions/memory/chunker.py` | ~150 | 决策语义分块 + 标签分类 |
| `tradingagents/extensions/memory/embedder.py` | ~110 | 双后端嵌入模型封装 |
| `tradingagents/extensions/memory/store.py` | ~200 | ChromaDB 存储后端 |
| `tradingagents/extensions/memory/retrieval.py` | ~250 | 混合检索管道 |
| `tradingagents/extensions/memory/provider.py` | ~210 | MemoryProvider 协议实现 |
| `tests/extension_memory_test/__init__.py` | — | 测试包 |
| `tests/extension_memory_test/conftest.py` | ~180 | 测试夹具 + 依赖检测 |
| `tests/extension_memory_test/test_agent_profiles.py` | ~130 | 角色配置 + 特征提取测试 |
| `tests/extension_memory_test/test_chunker.py` | ~120 | 分块 + 标签测试 |
| `tests/extension_memory_test/test_embedder.py` | ~55 | 嵌入模型测试 |
| `tests/extension_memory_test/test_store.py` | ~95 | 存储 CRUD 测试 |
| `tests/extension_memory_test/test_retrieval.py` | ~140 | 检索管道测试 |
| `tests/extension_memory_test/test_provider.py` | ~155 | 协议合规 + 集成测试 |
...
| `tradingagents/graph/trading_graph.py` | 类级 `memory_provider` 注入、`_resolve_pending_entries` 双路径、`_run_graph` 角色记忆检索、`_record_decision_via_provider`、`_retrieve_agent_memories` |
...
- 完整记忆扩展测试（含 chromadb + embedding）：`57 passed`
...
Provider 激活后，Agent 的初始 state 中会增加 `memory_{role}` 字段（字符串格式）。各 Agent 的 `create_xxx_node` 可以通过以下方式消费：

```python
memory = state.get("memory_market_analyst", "")
if memory:
    prompt = f"...\n- Relevant past experiences:\n{memory}\n..."
```

当前 Portfolio Manager 已自动获得 PM 角色记忆（映射到 `past_context`）。其他 Agent 的记忆字段已注入 state，等待 Agent 节点代码消费。

### 3. 新增角色

在 `agent_profiles.py` 的 `PROFILES` 字典中增加一行配置，在 `trading_graph.py` 的 `_retrieve_agent_memories` 的 `roles` 列表中增加角色名即可。不需要修改协议或存储层。

### 4. 可选依赖

`chromadb` 和 `sentence-transformers` 为基础依赖。本地嵌入模型 `all-MiniLM-L6-v2` 首次使用时会从 HuggingFace 下载（约 80MB），之后缓存到本地。国内用户可设置 `HF_ENDPOINT=https://hf-mirror.com` 使用镜像。
